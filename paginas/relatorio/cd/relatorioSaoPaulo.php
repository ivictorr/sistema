...
$filterFile = __DIR__ . "/filters/user_{$userId}_cdsp.txt";
...
<div class="table-container">
    ...
    if (isset($_POST['gerarRelatorio'])):
        $sql = "SELECT DISTINCT NE.NUM_DOCTO, A.Cod_filial_volume, A.Serie_volume, A.Num_volume, A.Qtde_pri, A.Qtde_aux, A.Valor_unitario, RTS.Num_docto, NE.Num_docto 
                FROM TBSAIDASITEMROM A 
                INNER JOIN TBSAIDAS RTS ON A.CHAVE_FATO = RTS.CHAVE_FATO AND RTS.COD_DOCTO = 'RTS' 
                INNER JOIN TBSAIDAS NE ON NE.CHAVE_FATO_ORIG_UN = RTS.CHAVE_FATO 
                INNER JOIN TBSAIDASITEM B ON A.CHAVE_FATO = B.CHAVE_FATO AND B.Num_subItem = 0 
                WHERE A.NUM_VOLUME IN ( 
                    SELECT DISTINCT A.NUM_VOLUME 
                    FROM TBSAIDASITEMROM A 
                    INNER JOIN TBSAIDAS RTS ON A.CHAVE_FATO = RTS.CHAVE_FATO AND RTS.COD_DOCTO = 'ROS' 
                    INNER JOIN TBSAIDASITEM B ON A.CHAVE_FATO = B.CHAVE_FATO AND B.Num_subItem = 0 
                    WHERE B.COD_LOCAL = '04' 
                     AND CAST(A.Datahora AS DATE) BETWEEN '$emissao_de' AND '$emissao_ate' 
                )";
        ...
        try {
            ...
            echo "<thead><tr><th>N° DOCTO</th><th>FILIAL</th><th>SÉRIE</th><th>N° VOLUME</th><th>QTDE PRI</th><th>QTDE AUX</th><th>VALOR UNITÁRIO</th><th>RTS DOCTO</th><th>NE DOCTO</th></tr></thead>";
            echo "<tbody>";
            
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                echo "<tr>";
                echo "<td>{$row['NUM_DOCTO']}</td>";
                echo "<td>{$row['Cod_filial_volume']}</td>";
                echo "<td>{$row['Serie_volume']}</td>";
                echo "<td>{$row['Num_volume']}</td>";
                echo "<td>{$row['Qtde_pri']}</td>";
                echo "<td>{$row['Qtde_aux']}</td>";
                echo "<td style='text-align: right;'>" . number_format($row['Valor_unitario'], 2, ',', '.') . "</td>";
                echo "<td>{$row['Num_docto']}</td>";
                echo "<td>{$row['NE.Num_docto']}</td>";
                echo "</tr>";
            }
            echo "</tbody></table>";
