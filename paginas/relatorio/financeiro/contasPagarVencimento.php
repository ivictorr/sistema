...
$filterFile = __DIR__ . "/filters/user_{$userId}_contapagvenc.txt";
...
<div class="container mt-5">
    ...
    <div class="panel panel-primary no-print mt-5" style="margin-top: 30px">
        <div class="panel-heading text-center">
            <h3>CONTAS A PAGAR POR VENCIMENTO</h3>
        </div>
        <br><br>
        <form method="POST" action="" class="form-horizontal">
            ...
            <div class="form-group row">
                <label for="formaPgto-select" class="col-sm-2 col-form-label text-right">Forma de Pagamento:</label>
                <div class="col-sm-10">
                    <select id="formaPgto-select" name="formaPgto[]" class="selectpicker form-control" multiple data-live-search="true" title="Selecione uma ou mais opções">';
                        <?php
...
<?php if (isset($_POST['gerarRelatorio'])): ?>
    <div id="filterSummary" class="filter-summary">
        ...
        <p><strong>Forma Pagamento:</strong>
            <?php
            echo isset($_POST['formaPgto']) && !empty($_POST['formaPgto'])
                ? implode(', ', $_POST['formaPgto'])
                : 'TODOS';
            ?>
        </p>
...
<div class="table-container">
    ...
    if (isset($_POST['gerarRelatorio'])):
        ...
        $formaPgto = isset($_POST['formaPgto']) ? $_POST['formaPgto'] : [];
    ...
    if (!empty($grupoEconomico)) {
        ...
        $sql = "SELECT ";
            $sql .= "A.Cod_filial as FILIAL, A.Cod_docto AS COD_DOCTO, A.Num_docto AS TITULO, A.Cod_banco_caixa AS CODIGO, B.Nome_agencia AS NOME, A.Valor_saldo AS SALDO, A.Valor_titulo AS VALOR, A.Data_emissao AS DATA_EMISSAO, A.Data_vencto AS DATA_VENCIMENTO, A.Cod_fornecedor AS FORNECEDOR, C.Nome_cadastro AS NOME_FORNECEDOR";
        $sql .= "
            FROM tbTituloPag A
            INNER JOIN tbBancoCaixa B ON A.Cod_banco_caixa = B.Cod_banco_caixa
            INNER JOIN TBCADASTROGERAL C ON A.Cod_fornecedor = C.Cod_cadastro
        ...
        if (!empty($formaPgto)) {
            $sql .= " AND A.Cod_forma_pgto IN ('" . implode("','", $formaPgto) . "')";
        }
        if (!empty($fornecedor)) {
            $sql .= " AND A.Cod_fornecedor IN ('" . implode("','", $fornecedor) . "')";
        }
        if (!empty($codCadastros)) {
            $sql .= " AND A.Cod_fornecedor IN ('" . implode("','", $codCadastros) . "')";
        }
        if (!empty($emissao_de)) {
